...
class HomeController extends Controller {
  async index() {
    ...
    const result = await this.app.mysql.select('stock_calculate_3b_f');
    result.map(item => {
    	item.count = JSON.parse(item.data).length
    	// delete item.data
    })

    const result2 = await this.app.mysql.select('stock_calculate_2b_f');
    result2.map(item => {
    	item.count = JSON.parse(item.data).length
    	// delete item.data
    })

    await this.ctx.render('home/indexf.tpl', {b3: JSON.stringify(result), b2: JSON.stringify(result2)});
  }
  async calculateZT () {
  ...
  async getStockData (date) {
  	const uri = `http://home.flashdata2.jrj.com.cn/limitStatistic/ztForce/${date.format('YYYYMMDD')}.js?_dc=1530422308647`
  ...
  async injectData (date) {
		const stock = await this.app.mysql.get('stock_f', { date: date.unix() });
		if (stock) {
			console.log(`${date.format('YYYYMMDD')}数据已经存在`)
		} else {
			...
			if (stockDataCode.indexOf('404 Not Found') != -1) {
				...
				let ztData = sandbox.yzb_ztForce.Data
				await this.app.mysql.insert('stock_f', { 
					date: date.unix(), 
  ...
  async getPrevDayStockData (prevDay) {
  	...
  	while (prevStock === null) {
  		prevStock = await this.app.mysql.get('stock_f', { date: prevDay.unix() });
  		if (!prevStock) prevDay.subtract(1, 'days')
  	}
  	return prevStock
  }
  async inject2BData (date) {
  	const result = await this.app.mysql.get('stock_calculate_2b_f', { date: date.unix() });
  	if (result) {
  		console.log(`${date.format('YYYYMMDD')}数据已经存在`)
  	} else {
			const B2Data = []
	  	const stockData = await this.app.mysql.get('stock_f', { date: date.unix() });
	  	if (!stockData) {
	  		console.log(`${date.format('YYYYMMDD')}数据不存在`)
				return;
	  	}
	  	const sotckList = JSON.parse(stockData.data);
	  	let prevDay = getPrevDay(date.clone())
	  	...
	  	await this.app.mysql.insert('stock_calculate_2b_f', { 
	  		date: date.unix(), 
  ...
  async inject3BData (date) {
  	const result = await this.app.mysql.get('stock_calculate_3b_f', { date: date.unix() });
  	if (result) {
  		console.log(`${date.format('YYYYMMDD')}数据已经存在`)
  	} else {
			const B3Data = []
	  	...
	  	console.log(`${date.format('YYYYMMDD')}插入`)
	  	await this.app.mysql.insert('stock_calculate_3b_f', { 
	  		date: date.unix(), 
